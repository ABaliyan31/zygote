...
Supported stacks (v1): `sanic`. More stacks (express, fastapi, django, react-vite, nextjs, vue, vanilla-html) planned — not yet implemented. If the requested stack isn't in this list, tell the user it's unsupported and list what's available. Do not attempt to scaffold an unlisted stack.
...
8. **Report**: print what was created, and the exact next command the user runs to start the app.
